...
use any_proxy::connector::{create_tls_config, Connector, TcpDialer};
...
async fn main() {
    ...
    // 5. 创建 Connector
    let connector = Arc::new(Connector::with_tls(
        SystemResolver::new(),
        (*policy).clone(),
        TcpDialer::new(),
        tls_config,
    ...
    let shutdown = shutdown_signal(config.shutdown_grace);
